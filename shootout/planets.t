...
local btype = cuda.make_struct_type(bodies[1])
local csqrt = cudalib.nvvm_sqrt_rm_d
local nbody = #bodies
terra hi(planets : &btype, i : int)
   var dt = 0.01
   var bi = planets[i]
   ...
   for j = 0, nbody do
      if i ~= j then
         ...
         var mag = csqrt(dx*dx + dy*dy + dz*dz)
         mag = dt / (mag * mag * mag)
         var bm = bj.mass*mag
         bivx = bivx - (dx * bm)
         bivy = bivy - (dy * bm)
